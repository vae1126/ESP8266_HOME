...
#define CMD_MAX_TOPIC_LEN   128
#define CMD_MAX_DATA_LEN    512

/* 命令类型 */
typedef enum {
    CMD_TYPE_LIGHT_RGB = 0,
    CMD_TYPE_LIGHT_SINGLE,
    CMD_TYPE_LIGHT_WS2812,
    CMD_TYPE_SWITCH,
    CMD_TYPE_DEVICE,
    CMD_TYPE_OTA,
} cmd_type_t;
...
typedef struct {
    cmd_type_t type;
    int index;
    char topic[CMD_MAX_TOPIC_LEN];
    int topic_len;
    char data[CMD_MAX_DATA_LEN];
    int data_len;
} mqtt_cmd_msg_t;
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
        ...
        {
            /* 防御性检查：topic和data必须有效 */
            if (event->topic_len == 0 || event->data_len == 0) {
                break;
            }

            /* 检查数据长度是否超出缓冲区 */
            if (event->topic_len >= CMD_MAX_TOPIC_LEN || event->data_len >= CMD_MAX_DATA_LEN) {
                ESP_LOGW(TAG, "MQTT message too large: topic=%d data=%d", event->topic_len, event->data_len);
                break;
            }

            /* 准备命令消息 */
            mqtt_cmd_msg_t cmd_msg = {0};
            memcpy(cmd_msg.topic, event->topic, event->topic_len);
            cmd_msg.topic_len = event->topic_len;
            memcpy(cmd_msg.data, event->data, event->data_len);
            cmd_msg.data_len = event->data_len;

            /* 使用实际设备数量进行匹配，避免遍历空主题 */
            device_caps_t caps_match = get_device_capabilities();

            /* 匹配PWM RGB命令 */
            if (caps_match.has_pwm_rgb) {
                for (int i = 0; i < caps_match.pwm_rgb_count && i < MAX_ENTITIES; i++) {
                    if (rgb_command_topic[i][0] != '\0' &&
                        event->topic_len == strlen(rgb_command_topic[i]) &&
                        strncmp(event->topic, rgb_command_topic[i], event->topic_len) == 0) {
                        cmd_msg.type = CMD_TYPE_LIGHT_RGB;
                        cmd_msg.index = i;
                        xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                        return ESP_OK;
                    }
                }
            }
            /* 匹配单色灯命令 */
            if (caps_match.has_pwm_single) {
                for (int i = 0; i < caps_match.pwm_single_count && i < MAX_ENTITIES; i++) {
                    if (single_command_topic[i][0] != '\0' &&
                        event->topic_len == strlen(single_command_topic[i]) &&
                        strncmp(event->topic, single_command_topic[i], event->topic_len) == 0) {
                        cmd_msg.type = CMD_TYPE_LIGHT_SINGLE;
                        cmd_msg.index = i;
                        xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                        return ESP_OK;
                    }
            ...
            /* 匹配WS2812命令 */
            if (caps_match.has_ws2812) {
                for (int i = 0; i < caps_match.ws2812_count && i < MAX_ENTITIES; i++) {
                    if (ws2812_command_topic[i][0] != '\0' &&
                        event->topic_len == strlen(ws2812_command_topic[i]) &&
                        strncmp(event->topic, ws2812_command_topic[i], event->topic_len) == 0) {
                        cmd_msg.type = CMD_TYPE_LIGHT_WS2812;
                        cmd_msg.index = i;
                        xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                        return ESP_OK;
                    }
            ...
            /* 匹配开关命令 */
            if (caps_match.has_switch) {
                for (int i = 0; i < caps_match.switch_count && i < MAX_ENTITIES; i++) {
                    if (switch_command_topic[i][0] != '\0' &&
                        event->topic_len == strlen(switch_command_topic[i]) &&
                        strncmp(event->topic, switch_command_topic[i], event->topic_len) == 0) {
                        cmd_msg.type = CMD_TYPE_SWITCH;
                        cmd_msg.index = i;
                        xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                        return ESP_OK;
                    }
            ...
            /* 匹配设备命令 */
            if (device_command_topic[0] != '\0' &&
                event->topic_len == strlen(device_command_topic) &&
                strncmp(event->topic, device_command_topic, event->topic_len) == 0) {
                cmd_msg.type = CMD_TYPE_DEVICE;
                cmd_msg.index = 0;
                xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                return ESP_OK;
            }
            /* 匹配OTA命令 - 使用精确的主题长度匹配 */
            if (ota_notify_topic[0] != '\0' &&
                event->topic_len == strlen(ota_notify_topic) &&
                strncmp(event->topic, ota_notify_topic, event->topic_len) == 0) {
                cmd_msg.type = CMD_TYPE_OTA;
                cmd_msg.index = 0;
                xQueueSend(mqtt_cmd_queue, &cmd_msg, 0);
                return ESP_OK;
            }
 ...
 * @brief  独立的命令处理任务
 *
 * 从队列中读取MQTT命令并处理，避免在MQTT回调中执行耗时操作
 * 这样可以：
 * 1. 不阻塞MQTT事件循环
 * 2. 避免在回调中调用vTaskDelay
 * 3. 提高系统响应性
 */
static void mqtt_cmd_process_task(void *pvParameters)
{
    ESP_LOGI(TAG, "MQTT command processing task started");

    mqtt_cmd_msg_t cmd_msg;
    ...
    while (1) {
        ...
        /* 从队列读取命令，阻塞等待 */
        if (xQueueReceive(mqtt_cmd_queue, &cmd_msg, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ESP_LOGD(TAG, "Processing command type=%d index=%d", cmd_msg.type, cmd_msg.index);

            switch (cmd_msg.type) {
                case CMD_TYPE_LIGHT_RGB:
                    handle_light_command(0, cmd_msg.index, cmd_msg.data, cmd_msg.data_len);
                    break;

                case CMD_TYPE_LIGHT_SINGLE:
                    handle_light_command(1, cmd_msg.index, cmd_msg.data, cmd_msg.data_len);
                    break;

                case CMD_TYPE_LIGHT_WS2812:
                    handle_light_command(2, cmd_msg.index, cmd_msg.data, cmd_msg.data_len);
                    break;

                case CMD_TYPE_SWITCH:
                    handle_switch_command(cmd_msg.index, cmd_msg.data, cmd_msg.data_len);
                    break;

                case CMD_TYPE_DEVICE:
                    handle_device_command(cmd_msg.data, cmd_msg.data_len);
                    break;

                case CMD_TYPE_OTA:
                    ota_upgrade_handle_command(cmd_msg.data, cmd_msg.data_len);
                    break;

                default:
                    ESP_LOGW(TAG, "Unknown command type: %d", cmd_msg.type);
                    break;
            }
        }
    }
...
void mqtt_ha_init(void)
{
    ...
    /* 创建命令队列 */
    mqtt_cmd_queue = xQueueCreate(CMD_QUEUE_SIZE, sizeof(mqtt_cmd_msg_t));
